...
220     continue
        if((UZTWC/UZTWM).ge.(UZFWC/UZFWM)) goto 225
...
!       Escoamento superficial	
        SSUR=0.0; S2SUR=0.0 
!       Escoamento interno (subsuperficial)	
        SIF=0.0; S2IF=0.0                           
!       Percolação
        SPERC=0.0     
!       Run-off direto	
        SDRO=0.0; S2DRO=0.0                          
!       Escoamento de base da zona livre inferior primária
        SPBF=0.0                                                
...
!        FI=Mudança no escoamento interno devido ao solo congelado
         if((PINC+UZFWC).gt.0.010) goto 251
         UZFWC=UZFWC+PINC
         goto 249
...
!        LZFSC(excesso)=LZFSC(real)+PERCS
...
249      continue
         ...
         if(ADIMC.le.(UZTWM+LZTWM)) goto 247
